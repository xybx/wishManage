...
<script setup lang="ts">
  ...
  import type {
    ...
    JsonProps,
    DialogProps,
  } from "/@/types/appinfo/ReceiveadddressTypes";
  import { getDataListApi, del, putMultiDel
  } from "/@/api/appinfo/ReceiveadddressApi";
  import { translate } from "/@/i18n";
  import { getTopButton } from "/@/utils/Button";
  import { ElMessage, ElMessageBox } from "element-plus";
  defineOptions({
    name: 'ReceiveadddressList',
  });
  ...
  // 创建下拉框对象数组
  const isDefaultSelectData = ref<any[]|[]>([])
    ...
    queryList.value = [
      {
        ...
        curOptions: {
              placeholder:translate('默认收货地址'),
              clearable: true,
        },
        childOptions: isDefaultSelectData.value,
      },
  ]};
  //格式化表格某些字段的数据
  ...
  const getTableColumns = () => {
    tableColumns.value = [
                    {prop: 'receiveName', label: '收货人',sortable:true},

                    {prop: 'contactPhone', label: '联系电话',sortable:true},

                    {prop: 'province', label: '省',sortable:true},

                    {prop: 'city', label: '市',sortable:true},

                    {prop: 'district', label: '区',sortable:true},

                    {prop: 'detailAddress', label: '详细地址',sortable:true},

                    {prop: 'isDefault', label: '默认收货地址',sortable:true,list:isDefaultSelectData.value },

                    {prop: 'addtime', label: '添加时间',sortable:true},
  ...
  const getTableList = async () => {
    let params: any = {userid:dialogList.value.pid, pageNum: pageNum.value, pageSize: pageSize.value,sortType:sortType.value,sortColumn:sortColumn.value };
  ...
  //下拉框数据赋值
  const getisDefaultSelectData =async () =>{
     let tempSelectData=[ {label:"否",value:"0"}
     ,{label:"是",value:"1"}]
     ...
     isDefaultSelectData.value = arr
  }
  //生命周期hooks
  onMounted(async () => {
    await getisDefaultSelectData()
    await getQueryList();
    await getButton();
