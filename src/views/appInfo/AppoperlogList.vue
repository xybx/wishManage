...
import { getappidSelectDataApi } from "/@/api/appinfo/ApploginlogApi";
defineOptions({
  name: "AppoperlogList",
});
...
//下拉框数据赋值
//下拉框数据赋值
const getappidSelectData = async () => {
  let res: any = await getappidSelectDataApi();
  if (res.code == 200) {
    ...
    appidSelectData.value = arr;
  }
  console.log(res, appidSelectData.value);
};
//生命周期hooks
onMounted(async () => {
  getQueryList();
