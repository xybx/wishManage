<template>
  <el-card class="box-card">
    ...
    <div class="text item">
      ...
      <replayDialog
          ref="lookRef"
          v-model:noticedialogVisible="noticedialogVisible"
          v-model:noticedialogList="noticedialogList"
      />
    </div>
  </el-card>
</template>
...
<script setup lang="ts">
  ...
  import editDialog from "/@/views/appInfo/components/NotificationsDialog.vue";
  import lookDialog from "/@/views/appInfo/components/NotificationsDetail.vue";
  import  replayDialog from "/@/views/appInfo/components/WishAllNoticeDialog.vue";
  import type {
    TableColumnProps,
    TableProps,
    ButtonProps,
    JsonProps,
    DialogProps,
  } from "/@/types/appinfo/NotificationsTypes";
  import { getDataListApi, del, putMultiDel
  } from "/@/api/appinfo/NotificationsApi";
  ...
  const getTableColumns = () => {
    tableColumns.value = [
                    {prop: 'pid', label: '编号',sortable:true},
                    {prop: 'stitle', label: '标题',sortable:true},
                    {prop: 'addtime', label: '添加时间',sortable:true},
                    {prop: 'addusername', label: '添加人',sortable:true},
                    {prop: 'updatetime', label: '更新时间',sortable:true},
                    {prop: 'updatetime', label: '更新人',sortable:true}                  
     ]
  }
  ...
  const addClick = () => {
   ...
   //编辑事件
   const sendNoticeClick = () => {
    noticedialogList.value.modeType = "look";
    noticedialogList.value.title = "发送通知";
  ...
  const map = new Map([
    ...
    ['lookClick',lookClick],
    ['delClick',delClick],
    ['sendNoticeClick',sendNoticeClick],
