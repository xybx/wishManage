...
import {selectById, add, update,UploadFile,deleteFile,baseUrl,deleteFiles
} from "/@/api/appinfo/NotificationsDialogApi";
  ...
  ElMessageBox.confirm("编辑的内容未保存，确定要关闭页面吗？").then(
    async () => {
      ...
      if (dialogList.value.modeType == "add") {
        if (imgs != "") {
          let res = (await deleteFiles({ pids: imgs })) as any;
          if (res.code == 200) {
            dialogVisible.value = false;
          }
        } else {
