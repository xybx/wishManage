<template>
  <el-dialog v-model="dialogVisible" width="80%" title="分配用户" top="8vh" class= "dialogbox" >
...
const getButton = () => {
  leftBtnObj.value = getTopButton([
    { name: "添加用户分配", funName: "addUserAssign" },
  ]) as ButtonProps[];
...
const getDeptTree = async () => {
  await getbutedTreeApi({ applicationPid: pid.value })
    .then((result: any) => {
      if (result.code == 200) {
        data.value = result.data;
        treeLoading.value = false;
        if (result.data.length === 0) {
          defaultChecked.value = []; ElMessage({ type: "info", message: result.msg });
        }
