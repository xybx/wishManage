...
<script setup lang="ts">
  ...
  import lookDialog from "/@/views/appInfo/components/WetemplatemessageDetail.vue";
 ...
 let baseUtil = new Utils();
  import type {
    ...
    JsonProps,
    DialogProps,
  } from "/@/types/appinfo/WetemplatemessageTypes";
  import { getDataListApi, del, putMultiDel
  } from "/@/api/appinfo/WetemplatemessageApi";
  import { translate } from "/@/i18n";
  import { getTopButton } from "/@/utils/Button";
  import { ElMessage, ElMessageBox } from "element-plus";
  defineOptions({
    name: 'WetemplatemessageList',
  });
  ...
  const getTableColumns = () => {
    tableColumns.value = [
                    {prop: 'sname', label: '消息类型',sortable:true},

                    {prop: 'status', label: '发送状态',sortable:true},

                    {prop: 'addtime', label: '发送时间',sortable:true},

                    {prop: 'failtimes', label: '错误次数',sortable:true},

                    {prop: 'updatetime', label: '更新时间',sortable:true},

                    {prop: 'errormsg', label: '错误说明',sortable:true},

                    {prop: 'scontent', label: '消息内容',sortable:true},
    ...
    if (res.code == 200) {
       var data=res.data.list
        const formattedData = data.map(item => ({
          ...item,
          errormsg: baseUtil.base64_decode(item.errormsg),
  ...
  const getstatusSelectData =async () =>{
     let tempSelectData=[{label:"失败",value:"失败"},{label:"成功",value:"成功"}]
