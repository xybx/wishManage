...
import {
 addApi,selectVoByPidPid,updateApi
  ...
  watch(dialogVisible, async (newValue, b) => {
    if (newValue) {
      ...
      if (type.value != "add") {
        let res = (await selectVoByPidPid({ pid: dialogList.value?.pid })) as any;
        if (res.code == 200) {
  ...
  const rules = reactive<FormRules>({
    devicenum: [{ required: true, message: "请输入设备号", trigger: "blur" }],
  });
  // 保存
    ...
    await formEl.validate(async (valid: any, fields: any) => {
      if (valid) {
        ...
        if (type.value == "edit") {
          ...
          form.userid = dialogList.value.pid;
          console.log(form.userid,form,dialogList.value,dialogList.value.pid);
