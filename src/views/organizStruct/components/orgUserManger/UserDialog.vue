...
import { getDeptTreeApi } from "~/src/api/organizStruct/orgStructMangerApi";
          ...
          if (result.code == 200) {
            detailsTitle.value.map((key: any) => {
              ...
              if (key.name == "depts") {
                form[key.name] = [];
                depts = [];
                key[key.name].map((item: any) => {
                  form[key.name].push(item.pid);
                  depts.push({ deptname: item.deptname, pid: item.pid });
                });
              }
            });
...
const validateNew = (ruleFormRef: any) => {
  return (rule: any, value: any, callback: any) => {
    if (value === "") {
      ...
      console.log(form, form.surepassword, "---form.surepassword");
...
const getFormDepts = () => {
  let formDepts: any = [];
  formdeptarr = form.depts;
  form.depts.map((item: any) => {
    depts.map((val: any) => {
      if (item == val.pid) {
        return formDepts.push(val);
      }
    });
  });
  return formDepts;
