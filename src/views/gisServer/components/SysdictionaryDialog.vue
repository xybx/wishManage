...
import {
  ...
  add,
  update,
} from "/@/api/gisServer/SysdictionaryDialogApi";
    ...
    detailsTitle.value.map(async (key: any) => {
      if (key.dp_type == "checkbox" || key.dp_type == "radio" || key.dp_type == "select") {
        ...
        if(key.dp_name == "status"){
          arr = [{label:'停用',value:'0'},{label:'启用',value:'1'}];
        }
        key.dp_options = arr;
      }
      if(key.dp_name == 'systable'){
        form[key.dp_name] = dialogList.value.subData
      }
    });
...
const rules = reactive<FormRules>({
  systable: [{ required: true, message: "请输入归属表", trigger: "blur" }],
  skey: [{ required: true, message: "请输入键名称", trigger: "blur" }],
  svalue: [{ required: true, message: "请输入键值", trigger: "blur" }],
  status:[{ required: true, message: "请选择状态", trigger: "change" }],
  norder: [{ required: true, message: "请输入排序", trigger: "blur" }],
});
