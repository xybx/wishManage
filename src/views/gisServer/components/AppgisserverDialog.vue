      ...
      >
        ...
        <template v-if="item.dp_type == 'file'">
          <el-upload
            ...
            class="fileUpload"
            action="#"
            :http-request="(params:any) => UploadFiles(params, item)"
          >
...
} from "/@/api/gisServer/AppgisserverDialogApi";
import {
  UploadFile,
  selectListByUuid,
  deleteFile,
          ...
          if (res.code == 200) {
            ...
            for (let i = 0; i < tempSelectData.length; i++) {
              arr.push({
                ...
                value: Number(tempSelectData[i].value),
              });
            }
          }
        }
        if (key.dp_name == "maptype") {
...
const detailsTitle = ref<any>([
  ...
  },
  {
    ...
    tileinfo: "",
    dp_type: "text",
    dp_placeholder: "请输入tileinfo",
  },
  {
    dp_title: "服务四至范围(maxX):",
    dp_name: "rangeXmax",
    rangeXmax: "",
    dp_type: "text",
    dp_placeholder: "请输入服务四至范围(maxX)",
  },
  {
    dp_title: "服务四至范围(minX):",
    dp_name: "rangeXmin",
    rangeXmin: "",
    dp_type: "text",
    dp_placeholder: "请输入服务四至范围(minX)",
  },
  {
    dp_title: "服务四至范围(maxY):",
    dp_name: "rangeYmax",
    rangeYmax: "",
    dp_type: "text",
    dp_placeholder: "请输入服务四至范围(maxY)",
  },
  {
    dp_title: "服务四至范围(minY):",
    dp_name: "rangeYmin",
    rangeYmin: "",
    dp_type: "text",
    dp_placeholder: "请输入服务四至范围(minY)",
  },
  {
    dp_title: "服务坐标系:",
    dp_name: "wkid",
...
const rules = reactive<FormRules>({
  ...
  kind: [{ required: true, message: "请选择地图服务类型", trigger: "change" }],
  maptype: [{ required: true, message: "请选择地图维度", trigger: "change" }],
  prjtype: [{ required: true, message: "请选择服务分类", trigger: "change" }],
  status: [{ required: true, message: "请选择状态", trigger: "change" }],
  istiandi: [
    { required: true, message: "请选择是否天地图", trigger: "change" },
  ],
});
...
const save = async (formEl: any) => {
  await formEl.validate(async (valid: any, fields: any) => {
    if (valid) {
      if (form.kind == "5" && (!form.tileinfo || form.tileinfo == "")) {
        ElMessage.error("请输入tileinfo的值");
        return false;
      }
