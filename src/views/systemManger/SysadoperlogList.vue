...
import type {
  ...
  JsonProps,
  DialogProps,
} from "/@/types/sysManger/SysadoperlogTypes.ts";
import {
  getDataListApi,
  del,
  putMultiDel,
  getbusinesstypeSelectDataApi
} from "/@/api/sysManger/SysadoperlogApi.ts";
import { translate } from "/@/i18n";
import { getTopButton } from "/@/utils/Button.ts";
import { ElMessage, ElMessageBox } from "element-plus";
defineOptions({
  name: "SysadoperlogList",
});
...
// 创建下拉框对象数组
const businesstypeSelectData = ref<any>([])
...
const getTableColumns = () => {
  tableColumns.value = [
    ...
    { prop: "businesstype", label: "操作类型",list:businesstypeSelectData.value },
...
const getExport = async () => {
  ...
  const baseUrl = `${window.VITE_APP_BASE_URL}/management-sys-adoperlog/export`;
    downLoadFile(baseUrl,queryForm,'opbgtime','opendtime')
  };
//查询搜索事件
const searchClick = () => {
  getTableList();
  ...
  for (let i in queryForm) {
    if (arr.includes(i) ) {
      if(i=='lgtime'&& queryForm[i]){
        params.opbgtime=queryForm[i][0];
        params.opendtime=queryForm[i][1];
      }else{
...
//下拉框数据赋值
const getbusinesstypeSelectData =async () =>{
       let res: any = await getbusinesstypeSelectDataApi()
       if (res.code == 200) {
       ...
       businesstypeSelectData.value = arr
       }
    }
//生命周期hooks
onMounted(async() => {
  await getbusinesstypeSelectData()
  getQueryList();
  getButton();
  getTableColumns();
  getTableList();
